...
export async function getServerSideProps({ params }) {
  // params.place === DB place.slug (예: seoul-budae-dae-sa-gwan)
  const place = await prisma.place.findUnique({
    where: { slug: params.place },
    include: {
      region: true,
...
export default function PlaceDetail({ place }) {
  ...
  const regionSlug = place.region?.slug || router.query.region;
  const addressText = place.address || "";
  const ratingText = (place.avgRating || 0).toFixed(1);

  const shareUrl = useMemo(() => {
    // 현재 페이지 주소(SSR/CSR 모두 안전)
    if (typeof window !== "undefined") return window.location.href;
    // SSR에선 대충 구성해줌 (정확 URL은 CSR에서 덮어씀)
      ...
      <section className="p-4 sm:p-6 -mt-6 sm:-mt-8 relative">
        <div className="rounded-2xl border bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            {/* 리뷰 작성은 기존 라우트 유지: 필요시 2단계 라우트로 변경 */}
            <Link
