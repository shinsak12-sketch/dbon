...
export default function Admin() {
  ...
  const [evForm, setEvForm] = useState({
    // 요구사항 반영 필드: 부서/담당자/기간/품격/오픈여부/방식/보정/개요
    org: "", // 주관부서
    title: "", // 대회명
    manager: "", // 부서담당자
    beginAt: "", // 시작일시
  ...
  async function loadEvents() {
    const data = await safeGetJSON("/api/champ/admin/events", { items: [] });
  ...
  async function loadNotices() {
    const data = await safeGetJSON("/api/champ/admin/notices", { items: [] });
  ...
  async function loadRules() {
    const data = await safeGetJSON("/api/champ/admin/settings", null);
      ...
      <section className="rounded-2xl border bg-white">
        ...
        {open.bg && (
          <div className="p-5 pt-0 space-y-4">
            {bgUrl ? (
              <img src={bgUrl} alt="현재 배경" className="w-full rounded-xl border" />
            ) : (
              <p className="text-gray-600">현재 등록된 배경이 없습니다.</p>
            )}
            <Uploader
              label={savingBg ? "업로드 중…" : "이미지 업로드"}
          ...
          <div className="p-5 pt-0 space-y-4">
            ...
            <div className="grid gap-3">
              <select
                ...
                value={evForm.org}
                onChange={(e) => setEvForm((v) => ({ ...v, org: e.target.value }))}
              >
              ...
              <input
                ...
                placeholder="대회명"
                value={evForm.title}
                onChange={(e) => setEvForm((v) => ({ ...v, title: e.target.value }))}
              />

              <input
                className="border rounded-lg p-3"
                placeholder="부서담당자"
                value={evForm.manager}
                onChange={(e) => setEvForm((v) => ({ ...v, manager: e.target.value }))}
              />
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  type="datetime-local"
                  className="border rounded-lg p-3"
                  value={evForm.beginAt}
                  onChange={(e) => setEvForm((v) => ({ ...v, beginAt: e.target.value }))}
                />
                <input
                  type="datetime-local"
                  className="border rounded-lg p-3"
              ...
              <select
                className="border rounded-lg p-3"
                value={evForm.state}
                onChange={(e) => setEvForm((v) => ({ ...v, state: e.target.value }))}
              >
                {["개요", "오픈", "중지", "종료", "결과"].map((s) => (
              ...
              <select
                className="border rounded-lg p-3"
                value={evForm.mode}
                onChange={(e) => setEvForm((v) => ({ ...v, mode: e.target.value }))}
              >
              ...
              <select
                className="border rounded-lg p-3"
                value={evForm.adjust}
                onChange={(e) => setEvForm((v) => ({ ...v, adjust: e.target.value }))}
              >
            ...
            <div className="flex gap-2">
              ...
              {evForm.id && (
                <button onClick={resetEventForm} className="btn-outline">
                  새로작성
                </button>
              )}
            </div>
            ...
            <div className="border rounded-xl divide-y">
              {events.length === 0 && (
                <div className="p-4 text-sm text-gray-500">등록된 대회가 없습니다.</div>
              )}
              {events.map((e) => (
                <div key={e.id} className="p-3 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => deleteEvent(e.id)} className="btn-outline">
                      삭제
                    </button>
                  </div>
                </div>
        ...
        {open.notice && (
          <div className="p-5 pt-0 space-y-4">
            <input
              ...
              placeholder="제목"
              value={ntForm.title}
              onChange={(e) => setNtForm((v) => ({ ...v, title: e.target.value }))}
            />
            <textarea
              className="w-full border rounded-lg p-3"
              rows={4}
              placeholder="내용"
              value={ntForm.content}
              onChange={(e) => setNtForm((v) => ({ ...v, content: e.target.value }))}
            />
            <label className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                checked={ntForm.pinned}
                onChange={(e) => setNtForm((v) => ({ ...v, pinned: e.target.checked }))}
              />
              상단 고정
            </label>
            <div className="flex gap-2">
              <button onClick={saveNotice} className="btn-primary">
                {ntForm.id ? "수정" : "등록"}
              </button>
              {ntForm.id && (
                <button onClick={() => setNtForm({ id: null, title: "", content: "", pinned: false })} className="btn-outline">
                  새로작성
                </button>
              )}
            </div>
              ...
              {notices.map((n) => (
                <div key={n.id} className="p-3">
                  ...
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => editNotice(n)} className="btn-outline">
                      수정
                    </button>
                    <button onClick={() => deleteNotice(n.id)} className="btn-outline">
                      삭제
                    </button>
                  </div>
                </div>
                ...
                {[120, 100, 80].map((k) => (
                  <div key={k} className="mt-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPointRules((v) => ({ ...v, tier: { ...v.tier, [k]: Number(e.target.value) } }))
                      }
                    />
                  </div>
                ))}
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-gray-600">
                        닉: <b>{p.nickname}</b> · 핸디: {p.handicap ?? "-"} · 생성:{" "}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <button onClick={resetPassword} className="rounded-lg bg-yellow-500 text-black px-3 py-2 font-semibold">
                        비번 초기화
                      </button>
                      <button onClick={deleteParticipant} className="rounded-lg bg-rose-600 text-white px-3 py-2 font-semibold">
                        삭제
                      </button>
                    </div>
                  </li>
